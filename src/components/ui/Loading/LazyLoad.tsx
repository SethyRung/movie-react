...
export const LazyLoad = <P extends Record<string, unknown> = Record<string, unknown>>({
  ...
  const LazyComponent = useMemo(
    () =>
      lazy(() => {
        return new Promise<{ default: ComponentType<Record<string, unknown>> }>(
          (resolve, reject) => {
            const attemptLoad = (attempt: number) => {
              loader()
                .then((module) =>
                  resolve(module as { default: ComponentType<Record<string, unknown>> }),
                )
                .catch((error) => {
  ...
  const loadingFallback = useMemo(
    () =>
      fallback || (
        <LoadingScreen message="Loading component..." variant="minimal" showProgress={false} />
      ),
    [fallback],
  );
...
export const withLazyLoad = <P extends Record<string, unknown> = Record<string, unknown>>(
  ...
  const LazyComponent = (props: P) => (
    <LazyLoad<P> loader={importFunc} {...options} props={props} />
  );
